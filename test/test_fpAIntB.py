...
class TestFpAInt8(unittest.TestCase):
  ...
  def setUp(self) -> None:
    from cutlass_gemm import gemm_op_fpAIntB
    # import moe_ops
  ...
  def generate_weights(self, n, k, dtype, quant_type):
    ...
    if quantize:
      _,weight_quant,weight_scale = gemm_op_utils.symmetric_quantize_last_axis_of_batched_matrix(weights["weights_for_ft"].cpu(), 0 if quant_type == torch.int8 else 1)
      weights["weights_for_ft_scale"] = weight_scale.to(weights["weights_for_ft"].device)
      weights["weights_for_ft"] = weight_quant.to(weights["weights_for_ft"].device)
    else:
