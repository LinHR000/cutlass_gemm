...
template<typename T, 
         ...
         typename WarpShape,
         int stages>
void generic_int8_gemm_kernelLauncher_fp16_splitk(const int8_t*     A,
                                          const int8_t*     B,
                                          const float       alpha,
                                          const float       beta,
                                          T*                C,
                                          ...
                                          cudaStream_t      stream)
{
    ...
    using Gemm = cutlass::gemm::device::GemmSplitKParallel<ElementInput,
    ...
    cutlass::Status init_status = gemm_op.initialize(args, workspace.get());
    // auto init_status = gemm_op.initialize(args, workspace, stream);
    if (init_status != cutlass::Status::kSuccess) {
        throw std::runtime_error("cutlass cannot initialize, status: " +
                                 std::to_string((int) init_status));
    }

    status = gemm_op();
    if (status != cutlass::Status::kSuccess) {
        throw std::runtime_error("cutlass cannot run, status: fp16" +
                                 std::to_string((int) status));
    }
}
...
struct dispatch_stages_fp16_splitk {
...
struct dispatch_stages_fp16_splitk<T, arch, ThreadblockShape, WarpShape, 2> {
    static void dispatch(const int8_t*     A,
                        const int8_t*     B,
                        const float       alpha,
                        const float       beta,
    ...
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        generic_int8_gemm_kernelLauncher_fp16_splitk<T, arch, ThreadblockShape, WarpShape, 2>(A, 
                                                                                  B, 
                                                                                  alpha,
                                                                                  beta, 
                                                                                  C, 
...
struct dispatch_stages_fp16_splitk<T,
                       cutlass::arch::Sm80,
                       ThreadblockShape,
                       WarpShape,
                       Stages,
                        ...
                        cudaStream_t      stream)
    {
        ...
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        generic_int8_gemm_kernelLauncher_fp16_splitk<T, cutlass::arch::Sm80, ThreadblockShape, WarpShape, Stages>(A, 
                                                                                                    B, 
                                                                                                    alpha,
                                                                                                    beta, 
                                                                                                    C, 
...
template<typename T, typename arch, typename ThreadblockShape, typename WarpShape>
void dispatch_gemm_config_fp16_splitk(const int8_t*     A,
                        const int8_t*     B,
                        const float       alpha,
                        const float       beta,
                        T*                C,
                        ...
                        cudaStream_t      stream)
{
    ...
    switch (stages) {
        case 2:
            using DispatcherStages2 = dispatch_stages_fp16_splitk<T, arch, ThreadblockShape, WarpShape, 2>;
            ...
            break;
        case 3:
            using DispatcherStages3 = dispatch_stages_fp16_splitk<T, arch, ThreadblockShape, WarpShape, 3>;
            ...
            break;
        case 4:
            using DispatcherStages4 = dispatch_stages_fp16_splitk<T, arch, ThreadblockShape, WarpShape, 4>;
...
void dispatch_gemm_to_cutlass_fp16_splitk(const int8_t*     A,
                                        const int8_t*     B,
                                        const float       alpha,
                                        const float       beta,
                                        __half*           C,
                                        ...
                                        cudaStream_t      stream)
{
    ...
    if (tile_config == "CtaShape128x256x128_WarpShape64x64x128"){
            dispatch_gemm_config_fp16_splitk<__half, 
                                cutlass::arch::Sm80, 
                                            ...
                                            cudaStream_t      stream)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (sm_ >= 80 && sm_ < 90) {
        dispatch_gemm_to_cutlass_fp16_splitk(A,
                                            B,
                                            alpha,
                                            beta,
                                            C,
