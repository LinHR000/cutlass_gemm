...
template<typename T, 
         ...
         typename WarpShape,
         int stages>
void generic_int8_gemm_mix_kernelLauncher(T*     A,
                                          int8_t*     B,
                                          T*          bias,
                                          const float       alpha,
                                          const float       beta,
...
struct dispatch_stages_mix {
                        ...
                        cudaStream_t      stream)
    {
        ...
        throw std::runtime_error("[FT Error][dispatch_stages_mix::dispatch] " + err_msg);
    }
};

template<typename T, typename arch, typename ThreadblockShape, typename WarpShape>
struct dispatch_stages_mix<T, arch, ThreadblockShape, WarpShape, 2> {
    static void dispatch(T*     A,
                        int8_t*     B,
                        T*          bias,
                        const float       alpha,
    ...
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        generic_int8_gemm_mix_kernelLauncher<T, arch, ThreadblockShape, WarpShape, 2>(A, 
                                                                                  B, 
                                                                                  bias,
                                                                                  alpha,
                                                                                  beta, 
...
struct dispatch_stages_mix<T,
                        ...
                        cudaStream_t      stream)
    {
        ...
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        generic_int8_gemm_mix_kernelLauncher<T, cutlass::arch::Sm80, ThreadblockShape, WarpShape, Stages>(A, 
                                                                                                    B, 
                                                                                                    bias,
                                                                                                    alpha,
                                                                                                    beta, 
...
template<typename T, typename arch, typename ThreadblockShape, typename WarpShape>
void dispatch_gemm_config_mix(T*     A,
                        int8_t*     B,
                        T*          bias,
                        const float       alpha,
                        const float       beta,
                        ...
                        cudaStream_t      stream)
{
    ...
    switch (stages) {
        case 2:
            using DispatcherStages2 = dispatch_stages_mix<T, arch, ThreadblockShape, WarpShape, 2>;
            ...
            break;
        case 3:
            using DispatcherStages3 = dispatch_stages_mix<T, arch, ThreadblockShape, WarpShape, 3>;
            ...
            break;
        case 4:
            using DispatcherStages4 = dispatch_stages_mix<T, arch, ThreadblockShape, WarpShape, 4>;
            ...
            break;
        default:
            std::string err_msg = "dispatch_gemm_config_mix does not support stages " + std::to_string(stages);
            throw std::runtime_error("[FT Error][dispatch_gemm_config_mix] " + err_msg);
            break;
    }
}
template<typename T>
void dispatch_gemm_to_cutlass_fp16_mix(T*           A,
                                        int8_t*     B,
                                        T*                bias,
                                        const float       alpha,
                                        const float       beta,
                                        ...
                                        cudaStream_t      stream)
{
    ...
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        if (tile_config == "CtaShape128x128x64_WarpShape64x64x64"){
            dispatch_gemm_config_mix<T, 
                                cutlass::arch::Sm80, 
                                cutlass::gemm::GemmShape<128, 128, 64>,
                                cutlass::gemm::GemmShape<64, 64, 64>>( 
                A, B, bias,alpha, beta, C, m, n, k,stridea,strideb,stridec,workspace, workspace_bytes, stages, splitK, stream);
    }
                                    ...
                                    cudaStream_t      stream)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (sm_ >= 80 && sm_ < 90) {
        dispatch_gemm_to_cutlass_fp16_mix<__half>(A,
                                            B,
                                            bias,
                                            alpha,
                                            beta,
                                    ...
                                    cudaStream_t      stream)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (sm_ >= 80 && sm_ < 90) {
        dispatch_gemm_to_cutlass_fp16_mix<__nv_bfloat16>(A,
                                            B,
                                            bias,
                                            alpha,
                                            beta,
