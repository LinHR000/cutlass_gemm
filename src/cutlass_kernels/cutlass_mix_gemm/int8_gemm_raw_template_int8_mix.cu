...
template<typename T, 
         ...
         typename WarpShape,
         int stages>
void generic_int8_gemm_mix_kernelLauncher(const T*     A,
                                          const int8_t*     B,
                                          const float       alpha,
                                          const float       beta,
                                          T*                C,
...
struct dispatch_stages_int8_mix {
...
struct dispatch_stages_int8_mix<T, arch, ThreadblockShape, WarpShape, 2> {
    static void dispatch(const T*     A,
                        const int8_t*     B,
                        const float       alpha,
                        const float       beta,
...
struct dispatch_stages_int8_mix<T,
                       cutlass::arch::Sm80,
                       ThreadblockShape,
                       WarpShape,
                       Stages,
...
template<typename T, typename arch, typename ThreadblockShape, typename WarpShape>
void dispatch_gemm_config_int8_mix(const T*     A,
                        const int8_t*     B,
                        const float       alpha,
                        const float       beta,
                        T*                C,
                        ...
                        cudaStream_t      stream)
{
    ...
    switch (stages) {
        case 2:
            using DispatcherStages2 = dispatch_stages_int8_mix<T, arch, ThreadblockShape, WarpShape, 2>;
            ...
            break;
        case 3:
            using DispatcherStages3 = dispatch_stages_int8_mix<T, arch, ThreadblockShape, WarpShape, 3>;
            ...
            break;
        case 4:
            using DispatcherStages4 = dispatch_stages_int8_mix<T, arch, ThreadblockShape, WarpShape, 4>;
...
template <typename T> 
void dispatch_gemm_to_cutlass_int8_mix(const T*     A,
                                const int8_t*     B,
                                const float       alpha,
                                const float       beta,
                                T*           C,
    ...
    if (tile_config == "CtaShape128x128x64_WarpShape64x64x64"){
            dispatch_gemm_config_int8_mix<T, 
                                cutlass::arch::Sm80, 
...
void cutlass_fp16_int8_gemm_per_tensor(const __half*     A,
                                    const int8_t*     B,
                                    const float       alpha,
                                    const float       beta,
                                    __half*           C,
                                    ...
                                    cudaStream_t      stream)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (sm_ >= 80 && sm_ < 90) {
        dispatch_gemm_to_cutlass_int8_mix<__half>(A,
                                            B,
                                            alpha,
                                            beta,
                                            C,
...
void cutlass_bf16_int8_gemm_per_tensor(const __nv_bfloat16*     A,
                                    const int8_t*     B,
                                    const float       alpha,
                                    const float       beta,
                                    __nv_bfloat16*           C,
