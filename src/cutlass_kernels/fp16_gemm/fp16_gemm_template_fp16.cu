...
template<typename T, 
         ...
         typename WarpShape,
         int stages>
void generic_fp16_gemm_kernelLauncher(const __half*     A,
                                        const __half*     B,
                                        const float       alpha,
                                        const float       beta,
                                        T*                C,
...
struct dispatch_stages {
    static void dispatch(const __half*    A,
                        const __half*     B,
                        const float       alpha,
                        const float       beta,
                        T*                C,
...
struct dispatch_stages<T, arch, ThreadblockShape, WarpShape, 2> {
    ...
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        generic_fp16_gemm_kernelLauncher<T, arch, ThreadblockShape, WarpShape, 2>(A, 
                                                                                  B, 
                                                                                  alpha,
                                                                                  beta, 
                                                                                  C, 
...
struct dispatch_stages<T,
                        ...
                        cudaStream_t      stream)
    {
        ...
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        generic_fp16_gemm_kernelLauncher<T, cutlass::arch::Sm80, ThreadblockShape, WarpShape, Stages>(A, 
                                                                                                    B, 
                                                                                                    alpha,
                                                                                                    beta, 
                                                                                                    C, 
...
template<typename T, typename arch, typename ThreadblockShape, typename WarpShape>
void dispatch_gemm_config(const __half*     A,
                        const __half*     B,
                        const float       alpha,
                        const float       beta,
                        T*                C,
...
void dispatch_gemm_to_cutlass_fp16_ofp16(const __half*     A,
                                        const __half*     B,
                                        const float       alpha,
                                        const float       beta,
                                        __half*           C,
                            ...
                            cudaStream_t      stream)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (sm_ >= 80 && sm_ < 90) {
        dispatch_gemm_to_cutlass_fp16_ofp16(A,
                                            B,
                                            alpha,
                                            beta,
                                            C,
