...
//
// "total_rows_before_expert" contains the index one past the last occurrence of the corresponding expert. (包含了相应专家最后一次出现的索引加一)
    ...
    const int64_t num_experts, int64_t* total_rows_before_expert)
{
    ...
    const int expert = blockIdx.x * blockDim.x + threadIdx.x; // 每个线程负责一个专家
    ...
    const int num_rows, const int64_t* num_valid_tokens_ptr, const int cols, const int k, cudaStream_t stream)
{
    ...
    const int threads = std::min(cols, 1024); // 每个 thread block 处理一行
    ...
    MOEExpertScaleNormalizationMode normalization_mode, cudaStream_t stream)
{
    ...
    sorter_.run((void*) sorter_ws_, sorter_ws_size_bytes, expert_for_source_row, permuted_experts_, source_rows_,
        permuted_rows_, k * num_rows, stream); // key,value 排序，key 是 topk 得到的 indices， 得到 sorted_indices

    // sync_check_cuda_error();

    // Upper bound on number of expanded rows
    const int expanded_active_expert_rows = k * active_rows; // active_rows == num_rows
